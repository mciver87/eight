      ...
      if ($this->csvHandle) {
        ...
        while ($row = $this->getNextCSVRow()) {
          ...
          if ($row['csvrownum'] != $this->headerRows - 1) {
            ...
            // We require a path for menu migrations.
            if ($row['path']) {
              $row['ref_parent'] = !empty($row['ref_parent']) ? $row['ref_parent'] : 0;
              $title_key = array_filter(array_slice($row, 2, 8));
